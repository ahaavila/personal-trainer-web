...
The system SHALL allow students to open and inspect a training plan to view its workout divisions (e.g. Treino A, Treino B) and each division's prescribed exercises, target sets, repetitions, rest interval, and notes.
...
The system SHALL display each exercise in the active workout with the prescribed number of sets, providing input fields for the student to record the actual weight (kg) and repetitions performed on each set, alongside the trainer's prescribed values as visual reference.
