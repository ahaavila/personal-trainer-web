...
function ExerciciosPage() {
  ...
  useEffect(() => {
    ...
    getExercises()
      ...
      .catch(() => {
        if (!cancelled) {
          setError('Não foi possível carregar a sua biblioteca de exercícios.')
          setLoading(false)
        }
      })
    return () => {
  ...
  return (
    <main className="exercises-page">
      <header className="exercises-page__header">
        <div>
          <h1>Exercícios</h1>
          <p>A sua biblioteca de movimentos para montar treinos consistentes.</p>
        </div>
        <Link className="exercises-page__new" to="/novo-exercicio">
          <Plus size={18} /> Novo exercício
        </Link>
      </header>
      <section className="exercises-library">
        <div className="exercises-filters">
          <label className="exercises-search">
            <Search size={18} />
            <input
              value={search}
        ...
        {loading ? (
          <p className="exercises-state">A carregar exercícios...</p>
        ) : error ? (
          ...
          <p className="exercises-state">Ainda não tem exercícios na sua biblioteca.</p>
        ) : filtered.length === 0 ? (
          <p className="exercises-state">Nenhum exercício corresponde aos filtros selecionados.</p>
        ) : (
            ...
            {filtered.map((exercise) => (
              ...
              >
                <div className="exercise-card__meta">
                  ...
                  <small>{LEVEL_LABELS[exercise.level]}</small>
                </div>
                <h2>{exercise.name}</h2>
                <p>{exercise.description}</p>
                <div className="exercise-card__defaults">
                  <div>
