...
import { Activity, ArrowRight, CalendarDays, Check, Dumbbell, Plus, Sparkles, Users } from 'lucide-react'
...
function PersonalDashboard({
  ...
  const activeCount = data.metrics.activeStudents
  const activeLabel = isPro ? `${activeCount}` : `${activeCount} / 5`
  const activeDetail = isPro ? 'Plano PRO · Ilimitado' : 'Limite de 5 no Plano Básico'

  const metrics = [
    { label: 'Alunos', value: data.metrics.students, detail: '+1 este mês', icon: Users },
    { label: 'Alunos ativos', value: activeLabel, detail: activeDetail, icon: Activity },
  ...
  return (
    <div className="dashboard-view">
      <div className="dashboard-view__header">
        <div>
          <span className="dashboard-view__date">{formatDate()}</span>
          <h1>Olá, {userName}.</h1>
          <p>Aqui está o panorama dos seus acompanhamentos hoje.</p>
        </div>
        <div className="dashboard-view__actions">
          ...
          <Link className="dashboard-button dashboard-button--muted" to="/novo-exercicio">
            <Plus size={17} /> Novo exercício
          </Link>
          <Link className="dashboard-button dashboard-button--primary" to="/criar-utilizador">
            <Users size={17} /> Adicionar aluno
          </Link>
        </div>
      </div>
      <div className="dashboard-metrics">
        {metrics.map(({ label, value, detail, icon: Icon }) => (
          <article className="dashboard-metric" key={label}>
            <span className="dashboard-metric__icon"><Icon size={18} /></span>
            <strong>{value}</strong>
            <h2>{label}</h2>
            <p>{detail}</p>
          </article>
      ...
      <div className="dashboard-grid">
        <section className="dashboard-section dashboard-section--schedule">
          <SectionHeader title="Próximos treinos" subtitle="Agenda organizada para hoje" />
          {data.upcomingTrainings.length === 0 ? (
            <p className="dashboard-empty">Nenhum treino agendado para hoje.</p>
          ) : (
            <div className="dashboard-training-list">
              {data.upcomingTrainings.map((training) => (
                <div className="dashboard-training" key={`${training.time}-${training.studentName}`}>
                  <time>{training.time}</time>
                  <span className="dashboard-training__avatar">{training.studentName.slice(0, 2).toUpperCase()}</span>
                  <div><strong>{training.studentName}</strong><p>{training.context}</p></div>
                  <span className="dashboard-status"><Check size={12} /> Confirmado</span>
                </div>
              ))}
            </div>
          )}
        </section>
        <section className="dashboard-section dashboard-section--evolution">
          <SectionHeader title="Evolução da semana" subtitle="Presenças registadas nos últimos dias" />
          <WeeklyBars points={data.weeklyEvolution} />
        </section>
      </div>
    </div>
  )
...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const promises: [Promise<DashboardData>, Promise<SubscriptionStatusResponse | null>] = [
      getDashboardForRole(user.role),
      user.role === 'personal' ? getSubscriptionStatus().catch(() => null) : Promise.resolve(null),
    ]

    Promise.all(promises)
      .then(([nextData, subData]) => {
        if (!cancelled) {
          setData(nextData)
  ...
  if (!data) return null
  return isPersonalData(data) ? (
    <PersonalDashboard data={data} userName={user.name} subscription={subscription} />
  ) : (
    <StudentDashboard data={data} userName={user.name} />
  )
}
